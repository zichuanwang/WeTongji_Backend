...
def gen_section(c_no, s_no)
	
end

p Rails.root

SeleCourse.select("student_no, course_no").group("course_no, student_no").where("created_at > '2013-09-01'").each do |item|
	p "#{item['student_no']}----#{item['course_no']}"
